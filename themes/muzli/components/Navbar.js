    ...
    <header className={`fixed w-full bg-white/80 backdrop-blur-md z-50 transition-all duration-300 py-4 ${scrolled ? 'shadow-md bg-white' : ''}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {menu.map(item => (
            <a key={item.name} href={item.href} className="font-medium hover:text-primary transition-colors relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 hover:after:w-full after:bg-primary after:transition-all after:duration-300">{item.name}</a>
          ))}
        </nav>
        {/* 功能按钮 */}
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t mt-4">
          <div className="container mx-auto px-4 py-3 space-y-3">
            {menu.map(item => (
              <a key={item.name} href={item.href} className="block py-2 font-medium hover:text-primary">{item.name}</a>
            ))}
